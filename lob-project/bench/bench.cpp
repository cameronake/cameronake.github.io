...
static LatencyResult measure_engine_latency(std::uint64_t seed) {
    ...
    // Estimate timer overhead so we can subtract it from each batch.
    double overhead = 1e9;
    ...
    // A single apply_msg() call is tens of ns -- well below the actual tick
    // of steady_clock on some hosts (it advertises a 1ns period, but the
    // real hardware resolution can be ~100ns, e.g. under a hypervisor-backed
    // QueryPerformanceCounter), which would quantize per-message timing into
    // useless round multiples of that tick. Instead, time fixed-size batches
    // of consecutive messages and record the batch's average per-message
    // cost; batch elapsed time is large enough that the tick no longer
    // dominates the measurement. Percentiles below are therefore over
    // per-batch averages (batches drawn from the same message-mix
    // distribution), not true single-message timings.
    constexpr std::size_t kBatch = 32;
    std::vector<double> lat;
    lat.reserve(fp.n);
    for (std::size_t i = 0; i < feed.size(); i += kBatch) {
        const std::size_t end = std::min(feed.size(), i + kBatch);
        const std::size_t n = end - i;
        auto t0 = Clock::now();
        for (std::size_t j = i; j < end; ++j) apply_msg(book, feed[j], trades);
        double batch_ns = ns_since(t0) - overhead;
        if (batch_ns < 0) batch_ns = 0;
        const double per_msg = batch_ns / static_cast<double>(n);
        for (std::size_t j = 0; j < n; ++j) lat.push_back(per_msg);
    }
...
int main(int argc, char** argv) {
    ...
    js += "\"meta\":{\"seed\":\"0xC0FFEE\",\"clock\":\"steady_clock\",\"note\":\"native build; latency = average cost per message over 32-message batches (host clock tick is coarser than a single apply_msg call), timer overhead subtracted per batch\"},";
