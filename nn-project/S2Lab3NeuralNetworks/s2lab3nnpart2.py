...
CUTOFF = 20 #number of repetitions to cut off after if total error does not go below 0.01
ERRORDIFF = 0.001
...
train_df = pd.read_csv("S2Lab3NeuralNetworks/mnist_train.csv")

output = [train_df.iloc[i]['5'] for i in train_df.index] 
train_df = train_df.iloc[:4000 , 1:]

input = [list(train_df.iloc[i]) for i in train_df.index]
output = [[[0, 1][val == n] for n in range(10)] for val in output]

indata = [input_line + [output_line] for input_line, output_line in zip(input, output)]

#recommended


#including input/output
layer_counts = [784, 48, 10]
...
laylists = [[random.random() * 4 - 2 for _ in range(numweights)] for numweights in weights_per_layer] 
#laylists = [[.15, .20, .25, .30], [.40, .45, .50, .55]]
#print(laylists)

wi_track = open("S2Lab3NeuralNetworks/wi_track.txt", "w")
acc_track = open("S2Lab3NeuralNetworks/acc_track.txt", "w")
error_track = open("S2Lab3NeuralNetworks/error_track.txt", "w")
...
def iterate(input_vals_lists, expected_outputs_lists, weights, biases, transformation):
    ...
    print("hi")
    wi_track.write("wi\tErrortotal")
    acc_track.write("Epoch\tAccuracy")
    error_track.write("Epoch\tErrortotal")
    oldloss = 1000000
    num_iterations = 0
    most_recently_stopped = 0
    while totalloss >= 0.01 and num_iterations < CUTOFF:
        if num_iterations % 1 == 0: print("iteration", num_iterations)
        if num_iterations % 1 == 0:
            wi_track.write("\n" + str(weights[-1][0][0]) + "\t" + str(totalloss))
            h, vals, eao = eval_and_find_errorlist(weights, biases, input_vals_lists, expected_outputs_lists, "T3")
            accuracy = sum([0, 1][e.index(max(e)) == o.index(max(o))] for e, o in eao) / len(eao)
            #print(eao)
            acc_track.write("\n" + str(num_iterations) + "\t" + str(accuracy))
            error_track.write("\n" + str(num_iterations) + "\t" + str(totalloss))
            #print(vals_for_loss)
